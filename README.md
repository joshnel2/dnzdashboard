...
All metrics are pulled from Clio's Reports API (CSV exports):
- **Revenue Report** for monthly deposits, weekly revenue, and YTD revenue
- **Productivity / Time Entries Detail Reports** for attorney billable hours and YTD time
...
## Clio Reports Integration

The dashboard requests CSV exports from Clio's Reports API and aggregates the results:

- **Revenue Report** (`/api/v4/reports/{category}/revenue.csv`)
  - Calculates monthly deposits, weekly revenue, and year-to-date revenue
  - Sums collected/payment/deposit columns across operating and trust accounts
  - Applies rolling 12-week window for weekly totals

- **Productivity & Time Entry Detail Reports**
  - Attorney billable hours are pulled from productivity reports where available
  - Falls back to time entry detail reports when productivity data is unavailable
  - YTD time is generated by summing recorded/billable hour columns per month

### Data Transformation

The `clioService` handles:
- Aggregating revenue columns (collected/payment/deposit amounts) and normalizing dates
- Selecting the most informative attorney-hours column across varying report schemas
- Calculating weekly revenue (last 12 weeks)
- Generating YTD monthly summaries for revenue and time
